...
export class ArtificialBassis {
    ...
    constructor(config) {
        ...
        // console.log(
        //     "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!",
        //     config.table[0].length,
        //     this.countLength(config.table)
        // );
        if (!config.in_calc) {
            for (
    ...
    start() {
        //console.log(" this.in_calc", this.in_calc);
    ...
    checkAnswer() {
        return new Promise((resolve, reject) => {
            let length = this.countLength(this.table),
                message = "";
            //console.log("checkAnswer", !this.checkPositiveDelta(this.table[length - 1]));
            this.getViewTable();
            if (!this.checkPositiveDelta(this.table[length - 1])) {
                this.kol_steps++;

                if (this.kol_steps < 15) {
                    //console.log("\n\nmust repead", this.mode_val, this.kol_steps);
                    this.newBasis();
                } else {
                    resolve({
                        status_failess: true,
                ...
                // || this.status_failess == true
                if (this.mode_val == "auto") {
                    // this.newBasis();
                    //console.log(this.mode_val, "this.mode_val AUTO^");
                    this.checkAnswer();
                } else {
                    message = "not auto";
                    if (this.checkPositiveDelta(this.table[length - 1])) {
                ...
                //console.log("safter new basis\n", this.new_basis, this.mode_val);

                resolve({ status_failess: true, message: message, data: this.table, basis: this.new_basis });
            } else {
                //console.log("before resolve", this.new_basis);
                this.removeAddTmp();
                //console.log("start resolve", this.new_basis);
                this.getViewTable();
    ...
    getCurentIndexStr(index_colum, check_error = true) {
        ...
        if (index_str == -1 && check_error) {
            //console.log("\n\n this.finish.message ERROR ", min, "index_colum", index_colum, index_str);
            this.getViewTable();
            this.finish.status_fail = true;
            this.finish.message = "Решения не существует";
        }
        //console.log("INDEx_str", index_str);
        return index_str;
    }
    //repeat
    ...
    newBasis() {
        ...
        // //console.log("!!!!!!111mode step\n", this.supp_index_col, this.supp_index_str);
        ...
        console.log(
            ...
            !this.checkPositiveDelta(this.table[this.countLength(this.table) - 1]),
            this.mode_val
        );
        ...
        if (!this.finish.status_fail && !this.checkPositiveDelta(this.table[this.countLength(this.table) - 1])) {
            if (this.mode_val == "auto") {
                ...
                //     index_str,
                //     "el=> ",
                //     this.table[index_str][index_colum].showNumber()
                // );
                //нужно тут это как то переграть .когда будем делатьобьеднение
                this.getViewTable();
                this.table = this.changeTable(index_colum, index_str);
                this.initBasisTemp(index_colum, index_str);
                this.newBasis();
    ...
    findDelta() {
        ...
        // //console.log("delta_cache", delta_cache);
    }
}
