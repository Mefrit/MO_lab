...
export class BasisSimplex {
    ...
    findDelta(bassis) {
        ...
        for (let j = 0; j < length; j++) {
            // console.log("findDelta!= ", this.delta_cache[j].showNumber(), " - ", this.function[j].showNumber());
    ...
    checkPositiveDelta(delta_cache) {
        ...
        for (let i = 0; i < delta_cache.length - 1; i++) {
            if (this.number_manager.toCompare(delta_cache[i], "<", 0)) res = false;
        }
        return res;
    }
    ...
    newBasis() {
        ...
        // console.log("MAINNNNNNNNNNNNNNNNNNNNnn", !this.finish.status_fail);
    ...
    initSupportEl(supp_index_str, supp_index_col) {
        ...
        console.log(this.supp_index_str, this.supp_index_col);
    }
